...
#define DELAY_BUF_SIZE 16000
 ...
 int16_t buf_ptr = 0;
 //float32_t *store_speech=(float32_t*) malloc(24000*sizeof(float32_t));
 ...
 audio_OUT = ((audio_chR<<16 & 0xFFFF0000)) + (audio_out_chL
& 0x0000FFFF);
 i2s_tx(audio_OUT);
 }
 ...
 int main(void)
 {
 audio_init ( hz24000, dmic_in, intr, I2S_HANDLER);
 while(1){}
 }